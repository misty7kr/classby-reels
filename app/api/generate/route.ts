...
규칙:
- 각 컷은 화면에 표시될 짧은 텍스트 (최대 2줄, 한 줄당 최대 12자)
- line1, line2 각각 절대 12자를 넘지 말 것 — 이것이 가장 중요한 규칙
- 줄바꿈은 반드시 의미 단위(어절)에서 자를 것 — 단어 중간에서 자르지 말 것
- 이모지 절대 금지
- 특수문자 최소화 (마침표, 쉼표, 물음표, 느낌표만 허용)
- 한국어로만 작성
- 컷 구조: [후킹] → [문제제기] → [원인/인사이트] → [해결책/차별점] → [CTA]
- 텍스트가 너무 길면 핵심 단어만 남기고 나머지는 삭제
